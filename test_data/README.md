...
These volumes are byte-identical copies of the MNI152 atlas fixture from
`test_data/ants_example/mni152.nii.gz`, as recorded in
`test_data/registration/README.md`. They are useful as sourced NIfTI-1 gzip
import fixtures and smoke-test inputs, but they are not a meaningful
registration-quality pair.

**Data source**: ANTs example MNI152 atlas, copied locally into
`test_data/registration/brain_fixed.nii.gz` and
`test_data/registration/brain_moving.nii.gz`.
...
|---|---|---|
| NIfTI-1 `.nii.gz` | `test_data/registration/brain_fixed.nii.gz`, sourced from the ANTs/MNI152 copy above | `ritk-nifti` imports the real repository gzip fixture and asserts ZYX shape, spacing, voxel count, non-zero content, and finite Float32 voxels |
