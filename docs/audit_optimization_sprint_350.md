...
**Original error** (audit §7.7 "Update" entry): `FlipImageFilter::new` expected `FlipPolicy` types but received `bool` types.

**Path note** — the file lives in the extracted `ritk-filter` crate at `crates/ritk-filter/src/transform/flip.rs` (the `transform/` subdirectory was moved during the Sprint 334+ `ritk-filter` extraction). The audit path `crates/ritk-core/src/filter/transform/flip.rs` predates the extraction.

**Fix** — `bool → FlipPolicy` migration with `pub enum FlipPolicy { Keep, Flip }`, `From<bool>` / `From<FlipPolicy>` impls, and a `from_bools` ctor for backward compatibility. Matches the workspace-wide "boolean blindness eliminated" pattern.

**Related audit blockers also verified resolved (2026-06-14)** — the `LabelId` / `ForegroundValue` migration in `ritk-snap` and `ritk-io` test sites, the `idft_real_into` import in `n4/tests_n4.rs` (now at `crates/ritk-filter/src/bias/n4/tests_n4.rs`), and the `clippy::needless_range_loop` in `slic/connectivity.rs` (now at `crates/ritk-segmentation/src/clustering/slic/connectivity.rs`) are all on-disk verified. One new blocker surfaced and fixed in the same pass: `warp.rs:101` `clippy::manual_div_ceil` → `n.div_ceil(chunk)`.

**Verification (2026-06-14)** — `cargo check --workspace` 0 errors in 57.98s; `cargo test --workspace --no-run` all test binaries build; `cargo clippy --workspace --all-features -- -D warnings` 0 errors, 0 warnings in 10.61s; `cargo test -p ritk-registration --lib deformable_field_ops` 32 passed, 0 failed. The historical `ritk-io.exe` linker file lock (audit §7.7) remains a Windows-environment concern, not a code defect.
