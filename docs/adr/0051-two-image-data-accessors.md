...
Revision 2026-09-23: Mainline already assigns ADR 0021 to GPU field-smoother
retirement, so this accepted decision moves to ADR 0051. Rebased call-site
inspection counts 62 removed-accessor references. The orthonormal fixture did
not distinguish a matrix inverse from a transpose; a nonsingular
nonorthogonal regression case now checks both directions. Geometry assertions
...
that must go carry **62 call sites**, not 681. The deferral was correct about
...
All 62 sites are migrated in this change, across every crate that had one. The
...
- `InverseDisplacementField::{apply, apply_native}` and
  ...
  their rebuild. The duplicate numerical cores now have one implementation per
  filter, and each entry point retains its format-specific extraction and
  reconstruction.
...
rewritten to that expression. Value-semantic workspace tests exercise the
changed callers; `cargo semver-checks` must report the five removals as the
declared public breaks they are.

The ten `CartesianGridGeometry` tests retain six behavior cases — forward and
inverse mapping, a fractional-coordinate round trip, displacement independent
of origin, identity-direction equality, and singular-direction rejection — and
add four: a 2-D instantiation, a nonsingular nonorthogonal direction, column
selection, and rejection of curvilinear coordinates. The 90-degree rotation
and shear fixtures use exact binary values; the fractional coordinates and
power-of-two spacings are exactly representable, so exact equality is the
appropriate oracle for these cases. The shear test distinguishes the inverse
from the transpose in both directions.
