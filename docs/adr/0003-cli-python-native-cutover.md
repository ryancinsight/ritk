...
Since ADR 0002 the **entire image-I/O layer reached native parity**
(MIG-493/494/495): all 9 formats (nifti, nrrd, analyze, mgh, metaimage, minc,
png, jpeg, tiff) have Atlas-native readers *and* writers behind the unified
`ImageReader<I>` / `ImageWriter<I>` contract, each verified byte-identical /
value-identical to its Burn counterpart. The `coeus` cargo feature was removed
(MIG-492): the native path is the mainline, always compiled and tested.
...
2. **Order commands by processing depth** (shallowest first — each step turns
   ...
   - **Phase A — pure I/O (no processing):** `convert`. Reads then writes; only
     ...
     jpeg}` (read+write) and `png` (read only — no writer for any substrate),
     while `dicom` and `vtk` have **no** native reader or writer. cli's
     ...
     **native-first with a Burn fallback for `dicom`/`vtk`**: the native
     helpers handle the 7 volume formats end-to-end, and inputs/outputs in
     `dicom`/`vtk` route to the existing Burn helpers until those formats gain a
     native path. This keeps `convert` fully functional while removing Burn from
     the common (volume-format) path.
   ...
   - **`viewer`:** GUI shell; cut over with or after its rendering path.
...
- **Convert native→Burn at the read boundary** (native reader → immediately
  build a Burn `Image`): rejected. It adds a conversion and removes *zero* Burn
  — the processing pipeline stays Burn-typed. It would let format-crate Burn
  *readers* die but not the Burn `Image` type or the processing surface, and it
  institutionalizes a copy on every load. Contrary to the migration goal.
...
## First increment (next slice)

**Phase A — native `convert`.** Add `read_image_native` / `write_image_native`
to `ritk-cli::commands` dispatching over the native `ImageReader`/`ImageWriter`
adapters (`ritk_io::format::<fmt>::native::*`) on `SequentialBackend`, route
`convert` through them for the 7 native volume formats with a Burn fallback for
`dicom`/`vtk`, and add a differential test (`convert` via native vs. Burn
produces byte-identical output). Change class [minor]. This validates the
per-command pattern end-to-end and is the template every later command follows.
