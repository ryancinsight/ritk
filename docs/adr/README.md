# ADR index

| ADR | Title | Status |
| --- | ----- | ------ |
| [0001](0001-coeus-native-registration-traits.md) | Coeus-native registration `Transform`/`Metric` trait surface | — |
| [0002](0002-core-image-burn-to-coeus-migration.md) | Core `Image` / tensor-substrate Burn→Coeus migration strategy | — |
| [0003](0003-cli-python-native-cutover.md) | CLI / Python consumer cutover to the native substrate | — |
| [0004](0004-trainable-displacement-field-cutover.md) | Trainable displacement-field native cutover | — |
| [0005](0005-static-displacement-native-boundary.md) | Static displacement and SSMMorph native boundary | — |
| [0006](0006-isolated-watershed-hierarchy.md) | Isolated watershed hierarchy ownership | — |
| [0007](0007-isolated-connected-native-ownership.md) | Isolated-connected native ownership | Accepted |
| [0008](0008-vector-confidence-native-ownership.md) | Vector confidence-connected native ownership | Accepted |
| [0009](0009-statistics-native-extrema.md) | Native statistics extrema boundary | — |
| [0010](0010-atlas-provider-checkout.md) | Atlas-owned provider checkout | — |
| [0011](0011-iris-visualization-provider.md) | Iris visualization-provider boundary | — |
| [0012](0012-fallible-jpeg-2000-encoder.md) | Native JPEG 2000 encoding contract | Accepted |
| [0013](0013-fallible-jpeg-ls-encoder.md) | Fallible native JPEG-LS encoder and rolling reconstruction rows | Accepted |
| [0014](0014-fallible-descriptive-statistics.md) | Fallible finite descriptive statistics | Accepted |
| [0015](0015-fallible-temporal-synchronization.md) | Fallible temporal synchronization | Accepted |
| [0016](0016-module-error-bridge.md) | Type-erased Coeus module-error bridge | Accepted |
| [0017](0017-diffusion-mri-pipeline.md) | Physically typed diffusion MRI pipeline | Accepted |
| [0018](0018-single-coordinate-transform-api.md) | One coordinate-transform API on `Image` | Accepted |
| [0019](0019-borrowed-voxel-view-seam.md) | Borrowed voxel views on `Image` | Proposed |
