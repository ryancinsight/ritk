...
Revision 2026-09-11 (browser visual smoke): the packaged RITK library from
merge `55ee03f1` was served from a local HTTP origin with the Métis consumer at
`f2aa19e2` and the merged Moirai canvas provider at `ccdc878d`. A synthetic
three-file Part 10 study generated by `dicom_workflow` was dispatched through a
browser `DataTransfer`; the host reported three accepted 654-byte files and
1,962 bytes read, and the RITK canvas reported `dicom-frame-visible` with a
non-black 4 × 2 RGBA frame displayed at 512 × 512 CSS pixels. The inspected PNG
is `docs/manual/images/dicom-metis-browser.png`. This is a real byte-to-frame
runtime smoke across RITK, Métis and Moirai, but the DOM event is untrusted and
...
portable executable and 9,424,896-byte per-user MSI pass inventory hash
validation. The portable executable opened the committed 410-file public CT
...
`88c60a0b6410c0e07700e965bcdbea43b7b20789`; the chooser and package workflows
