...
impl HumanAtlas {
    pub fn read(dataset: &HumanDataset) -> Result<Self> {
        ...
        // The label volume is queried in its own index space, so the grid is
        // the identity affine over that space rather than the scanner's.
        let grid = ParcellationGrid::axis_aligned(
            [columns, rows, depth],
            [1.0, 1.0, 1.0],
            [0.0, 0.0, 0.0],
        )
        .context("constructing the endpoint parcellation grid")?;
