...
fn main() -> Result<()> {
    ...
    let geometries = label_tracks
        ...
        .collect::<Vec<_>>();
    let connectome = ritk_connectome::build_connectivity_matrix(&atlas.parcellation, &geometries)
    ...
    let metrics = HumanMetrics {
        ...
        assigned_streamlines: connectome.total_streamlines() - connectome.skipped_count(),
        median_length_mm,
...
fn validate_connectome_accounting(matrix: &ritk_connectome::ConnectivityMatrix) -> Result<()> {
    ...
    let assigned = (matrix.total_streamlines() - matrix.skipped_count()) as f64;
    anyhow::ensure!(
        recorded_weight == assigned,
        "connectome accounting differs: edge weights sum to {recorded_weight}, assigned streamlines are {assigned}"
    );
