...
pub(crate) fn derive_image_geometry(
    ...
    let direction_matrix = direction_matrix_from_columns(direction_columns);
    let origin_vec = Vector3::new(c_ras[0] as f64, c_ras[1] as f64, c_ras[2] as f64)
        - centered_half_offset(direction_matrix, spacing, dims);
...
pub(crate) fn ras_center_from_geometry(
    ...
    Vector3::new(origin[0], origin[1], origin[2])
        + centered_half_offset(direction.0, spacing, dims_xyz)
}

fn direction_matrix_from_columns(columns: [[f32; 3]; 3]) -> SMatrix<f64, 3, 3> {
    SMatrix::<f64, 3, 3>::from_columns(&[
        Vector3::new(
            columns[0][0] as f64,
            columns[0][1] as f64,
            columns[0][2] as f64,
...
fn centered_half_offset(
    direction: SMatrix<f64, 3, 3>,
    spacing: Spacing<3>,
    dims_xyz: [usize; 3],
) -> Vector3<f64> {
    let half_dim = Vector3::new(
