...
pub(crate) fn interpolate_1d<B: Backend, const D: usize>(
    ...
    // Clamp indices (preserve x0 via .clone() so it is available for the zero-pad mask)
    ...
    if zero_pad {
        // A sample is in-bounds iff floor(c) == clamp(floor(c), 0, d-1).
        // x0.clone() preserves x0 for the final clamp (which consumes it).
        let x_in = x0.clone().equal(x0.clamp(0.0, (d0 - 1) as f64)).float();
        result * x_in
    } else {
        result
    }
