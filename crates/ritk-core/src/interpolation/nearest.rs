...
impl NearestNeighborInterpolator {
    fn interpolate_4d<B: Backend, const D: usize>(&self, data: &Tensor<B, D>, indices: Tensor<B, 2>) -> Tensor<B, 1> {
        ...
        // indices: (x, y, z, w)
        let x = indices.clone().slice([0..indices.dims()[0], 0..1]).flatten::<1>(0, 1);
        let y = indices.clone().slice([0..indices.dims()[0], 1..2]).flatten::<1>(0, 1);
        let z = indices.clone().slice([0..indices.dims()[0], 2..3]).flatten::<1>(0, 1);
        let w = indices.clone().slice([0..indices.dims()[0], 3..4]).flatten::<1>(0, 1);
