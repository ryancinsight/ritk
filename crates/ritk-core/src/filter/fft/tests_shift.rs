...
use crate::filter::fft::FftShiftFilter;
...
fn output_shape_unchanged_2d() {
    ...
    let shifted = FftShiftFilter::new().apply_2d(&img).unwrap();
...
fn self_inverse_2d() {
    ...
    let once = FftShiftFilter::new().apply_2d(&img).unwrap();
    let twice = FftShiftFilter::new().apply_2d(&once).unwrap();

    let (result, _) = crate::filter::ops::extract_vec(&twice).unwrap();
    let max_diff = result
        .iter()
        .zip(vals.iter())
        .map(|(a, b)| (a - b).abs())
...
fn dc_moves_to_center_2d() {
    ...
    let img = make_complex_2d(&vals, h, cw);
    let shifted = FftShiftFilter::new().apply_2d(&img).unwrap();
    let (shifted_data, _) = crate::filter::ops::extract_vec(&shifted).unwrap();
...
fn output_shape_unchanged_3d() {
    ...
    let shifted = FftShiftFilter::new().apply_3d(&img).unwrap();
...
fn dc_moves_to_center_3d() {
    ...
    let img = make_complex_3d(&vals, depth, h, cw);
    let shifted = FftShiftFilter::new().apply_3d(&img).unwrap();
    let (data, _) = crate::filter::ops::extract_vec(&shifted).unwrap();
...
fn self_inverse_3d() {
    ...
    let once = FftShiftFilter::new().apply_3d(&img).unwrap();
    let twice = FftShiftFilter::new().apply_3d(&once).unwrap();
