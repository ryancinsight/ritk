...
use crate::filter::fft::frequency_filter::{FftFilterKind, FrequencyDomainFilter};
...
fn constant_preserved_by_low_pass_2d() {
    ...
    let result = FrequencyDomainFilter::new()
        .apply_2d(&img, FftFilterKind::IdealLowPass, 0.3, 2)
        .unwrap();
...
fn constant_removed_by_high_pass_2d() {
    let img = make_image_2d(vec![42.0_f32; 64], 8, 8);
    let result = FrequencyDomainFilter::new()
        .apply_2d(&img, FftFilterKind::IdealHighPass, 0.3, 2)
        .unwrap();
    let (out, _) = extract_vec(&result).unwrap();
    for (i, &v) in out.iter().enumerate() {
        assert!(
...
fn checkerboard_attenuated_by_low_pass_2d() {
    ...
    let result = FrequencyDomainFilter::new()
        .apply_2d(&img, FftFilterKind::IdealLowPass, 0.2, 2)
        .unwrap();
    let (out, _) = extract_vec(&result).unwrap();
...
fn butterworth_low_pass_finite_2d() {
    ...
    let result = FrequencyDomainFilter::new()
        .apply_2d(&img, FftFilterKind::ButterworthLowPass, 0.3, 4)
        .unwrap();
    let (out, _) = extract_vec(&result).unwrap();
    for (i, &v) in out.iter().enumerate() {
...
fn butterworth_high_pass_finite_2d() {
    ...
    let img = make_image_2d(vals, 10, 10);
    let result = FrequencyDomainFilter::new()
        .apply_2d(&img, FftFilterKind::ButterworthHighPass, 0.3, 4)
        .unwrap();
    let (out, _) = extract_vec(&result).unwrap();
    for (i, &v) in out.iter().enumerate() {
    ...
    for bad_cutoff in &[0.0, -0.1, 0.6, 1.0] {
        assert!(
            FrequencyDomainFilter::new()
                .apply_2d(&img, FftFilterKind::IdealLowPass, *bad_cutoff, 2)
                .is_err(),
