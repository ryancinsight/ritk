...
use ritk_core::transform::{Transform, BSplineTransform};
...
fn test_bspline_transform_2d_shift() {
    ...
    assert!((actual[0] - 2.5).abs() < 1e-5, "Expected 2.5, got {}", actual[0]);
    assert!((actual[1] - 1.5).abs() < 1e-5, "Expected 1.5, got {}", actual[1]);
}
...
fn test_bspline_transform_3d_chunking() {
    ...
    let points_data = TensorData::from([
        [1.5, 1.5, 1.5],
        [2.0, 2.0, 2.0]
    ]);
    let points = Tensor::<B, 2>::from_data(points_data, &device);
...
fn test_bspline_boundary_conditions() {
    ...
    // Should remain -1.0
    assert!((slice_out[0] - -1.0).abs() < 1e-5, "Outside point X mismatch: {}", slice_out[0]);
    assert!((slice_out[1] - -1.0).abs() < 1e-5, "Outside point Y mismatch: {}", slice_out[1]);
