//! CMA-ES, multi-start, and multiscale rigid registration tests on RIRE Patient-001.
//!
...
fn test_cma_mi_multiscale_on_rire_patient001() {
    ...
    println!(
        "\n\u{2500}\u{2500} Running 3-level cascade CMA-ES (NMI, shrinks 16\u{2192}8\u{2192}4) \u{2500}\u{2500}"
    );
    ...
    println!("\n\u{2500}\u{2500} Multiscale Cascade Results \u{2500}\u{2500}");
    ...
    for (i, &r) in rot.iter().enumerate() {
        assert!(
            r.abs() <= rot_limit,
            "Rotation[{i}] = {r:.4} rad outside [\u{00b1}{rot_limit:.4}]"
        );
    }
    for (i, &t) in trans.iter().enumerate() {
        assert!(
            t.abs() <= trans_limit + 1.0,
            "Translation[{i}] = {t:.2} mm outside expected range \u{00b1}{trans_limit:.0} mm"
        );
    }

    if tre_final < tre_identity {
        println!(" \u{2713} TRE improved: {tre_identity:.2} mm \u{2192} {tre_final:.2} mm");
    } else {
        println!(" \u{26a0} TRE did not improve ({tre_identity:.2} \u{2192} {tre_final:.2} mm).");
        println!(" Expected behaviour: cascade finds better basin than single-level.");
    }

    println!("\n\u{2713} All multiscale cascade assertions passed.");
}
