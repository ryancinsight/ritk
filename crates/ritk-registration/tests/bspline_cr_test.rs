...
fn test_bspline_cr_registration_small() {
    ...
    // Decreased learning rate from 0.1 to 0.01 since gradient clipping was removed
    ...
    // Looser tolerance because 50 iterations might not converge fully,
    // but we want to ensure gradients moved it in the right direction.
    // If gradients were zero, error would be 1.0.
    assert!(err_x < 0.8, "Error X too high: {}", err_x);
    assert!(err_y < 0.8, "Error Y too high: {}", err_y);
}
