...
use ritk_registration::metric::{NormalizedMutualInformation, NormalizationMethod, Metric};
...
fn test_nmi_perfect_match() {
    ...
    let metric = NormalizedMutualInformation::<B>::new(
        32, 0.0, 1.0, 0.1, NormalizationMethod::JointEntropy
    );
    ...
    assert!(loss_val < -1.8, "Loss should be close to -2.0 for perfect match, got {}", loss_val);
}
