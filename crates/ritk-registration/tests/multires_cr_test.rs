...
fn test_multires_cr_registration() {
    ...
    let transform = TranslationTransform::<B, 3>::new(Tensor::zeros([3], &device).require_grad());
    ...
    // Customize iterations and learning rates for mathematically precise (non-clipped) optimization
    ...
    assert!((t_vals[0] - 0.0).abs() < 0.5, "Z error: {}", t_vals[0]);
    assert!((t_vals[1] - 4.0).abs() < 0.5, "Y error: {}", t_vals[1]);
    assert!((t_vals[2] - 4.0).abs() < 0.5, "X error: {}", t_vals[2]);
}
