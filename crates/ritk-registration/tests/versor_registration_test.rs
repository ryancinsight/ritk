...
use ritk_transform::VersorRigid3DTransform;
...
fn versor_registers_rotated_ellipsoid() {
    ...
    let rotation = Tensor::<f32, B>::from_floats([0.0, 0.0, 0.0, 1.0], &device);
    ...
    let dist_pos = (r_norm[0] - 0.0).powi(2)
        + (r_norm[1] - 0.0).powi(2)
        + (r_norm[2] - expected_z).powi(2)
        + (r_norm[3] - expected_w).powi(2);
    let dist_neg = (r_norm[0] + 0.0).powi(2)
        + (r_norm[1] + 0.0).powi(2)
        + (r_norm[2] + expected_z).powi(2)
        + (r_norm[3] + expected_w).powi(2);
