...
fn bench_displacement_evaluation(c: &mut Criterion) {
    ...
    let cache_small = BasisCache::new(VolumeDims(dims_small), &ctrl_spacing_small);
    ...
    let ctrl_dims_med = init_control_grid(VolumeDims(dims_med), &ctrl_spacing_med);
    ...
    let cache_med = BasisCache::new(VolumeDims(dims_med), &ctrl_spacing_med);
    ...
    let ctrl_dims_large = init_control_grid(VolumeDims(dims_large), &ctrl_spacing_large);
    ...
    let cache_large = BasisCache::new(VolumeDims(dims_large), &ctrl_spacing_large);
