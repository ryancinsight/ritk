...
//!    c. Compute mean forward velocity: vÌ„ = (1/N) Σᵢ vᵢ (component-wise).
//!    d. Template sharpening: T^k = warp(T̃^k, exp(−G_σ ∗ vÌ„)) where G_σ is
...
impl AtlasRegistration {
    ...
    pub fn build_atlas_with<B: ritk_image::tensor::Backend>(
        ...
        for k in 0..self.config.max_iterations {
            ...
            // 2c. Mean forward velocity vÌ„ = (1/N) Σᵢ vᵢ.
