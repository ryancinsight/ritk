...
fn dispatch_laplacian_squared<B: Backend, const D: usize>(
    ...
    match D {
        ...
        _ => panic!(
            "Laplacian-squared regularizer only supports D ∈ {{4, 5}}, got D = {D}"
        ),
    }
}
...
pub fn dispatch_diffusion<B: Backend, const D: usize>(
    ...
    match D {
        ...
        _ => panic!(
            "DiffusionRegularizer only supports D ∈ {{4, 5}}, got D = {D}"
        ),
    }
}
...
pub fn dispatch_elastic<B: Backend, const D: usize>(
    ...
    match D {
        ...
        _ => panic!(
            "ElasticRegularizer only supports D ∈ {{4, 5}}, got D = {D}"
        ),
    }
}
...
pub fn dispatch_total_variation<B: Backend, const D: usize>(
    ...
    match D {
        ...
        _ => panic!(
            "TotalVariationRegularizer only supports D ∈ {{4, 5}}, got D = {D}"
        ),
      }
    }
    ...
    mod tests {
      ...
      fn assert_finite(loss: f32, label: &str) {
        assert!(
          loss.is_finite(),
          "{label} should be finite, got {loss}"
        );
      }
      ...
      fn bending_energy_2d_zero_displacement_is_zero() {
        ...
        let loss: f32 = super::dispatch_bending_energy::<B, 4>(displacement, 1.0)
          .into_scalar();
      ...
      fn bending_energy_3d_zero_displacement_is_zero() {
        ...
        let loss: f32 = super::dispatch_bending_energy::<B, 5>(displacement, 1.0)
          .into_scalar();
      ...
      fn bending_energy_2d_nonzero_is_finite_and_positive() {
        ...
        assert!(loss >= 0.0, "bending_energy should be non-negative, got {loss}");
      }
      ...
      fn curvature_2d_zero_displacement_is_zero() {
        let displacement = Tensor::<B, 4>::zeros([1, 2, 4, 4], &Default::default());
        let loss: f32 = super::dispatch_curvature::<B, 4>(displacement, 1.0)
          .into_scalar();
      ...
      fn curvature_3d_zero_displacement_is_zero() {
        let displacement = Tensor::<B, 5>::zeros([1, 3, 4, 4, 4], &Default::default());
        let loss: f32 = super::dispatch_curvature::<B, 5>(displacement, 1.0)
          .into_scalar();
      ...
      fn diffusion_2d_zero_displacement_is_zero() {
        let displacement = Tensor::<B, 4>::zeros([1, 2, 4, 4], &Default::default());
        let loss: f32 = super::dispatch_diffusion::<B, 4>(displacement, 1.0)
          .into_scalar();
      ...
      fn diffusion_3d_zero_displacement_is_zero() {
        let displacement = Tensor::<B, 5>::zeros([1, 3, 4, 4, 4], &Default::default());
        let loss: f32 = super::dispatch_diffusion::<B, 5>(displacement, 1.0)
          .into_scalar();
      ...
      fn elastic_2d_zero_displacement_is_zero() {
        let displacement = Tensor::<B, 4>::zeros([1, 2, 4, 4], &Default::default());
        let loss: f32 = super::dispatch_elastic::<B, 4>(displacement, 1.0, 1.0)
          .into_scalar();
      ...
      fn elastic_3d_zero_displacement_is_zero() {
        let displacement = Tensor::<B, 5>::zeros([1, 3, 4, 4, 4], &Default::default());
        let loss: f32 = super::dispatch_elastic::<B, 5>(displacement, 1.0, 1.0)
          .into_scalar();
      ...
      fn total_variation_2d_zero_displacement_is_zero() {
        let displacement = Tensor::<B, 4>::zeros([1, 2, 4, 4], &Default::default());
        let loss: f32 = super::dispatch_total_variation::<B, 4>(displacement, 1.0)
          .into_scalar();
      ...
      fn total_variation_3d_zero_displacement_is_zero() {
        let displacement = Tensor::<B, 5>::zeros([1, 3, 4, 4, 4], &Default::default());
        let loss: f32 = super::dispatch_total_variation::<B, 5>(displacement, 1.0)
          .into_scalar();
      ...
      fn total_variation_2d_nonzero_is_finite_and_positive() {
        ...
        assert!(loss >= 0.0, "total_variation should be non-negative, got {loss}");
      }
