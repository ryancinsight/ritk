...
fn test_identity_registration_has_near_zero_mse() {
    ...
    let result = reg
        .register(&img, &img, dims, [1.0, 1.0, 1.0])
...
fn test_weight_zero_matches_standard_diffeomorphic() {
    ...
    let result_ic = reg_ic
        .register(&img, &img, dims, spacing)
        .expect("infallible: validated precondition");
...
fn test_ic_residual_decreases_with_symmetric_weight() {
    ...
    let result_fwd = reg_fwd
        .register(&img, &img, dims, [1.0; 3])
        .expect("infallible: validated precondition");
    let result_sym = reg_sym
        .register(&img, &img, dims, [1.0; 3])
        .expect("infallible: validated precondition");
