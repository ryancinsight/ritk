...
pub(crate) fn apply_affine_perturbation(
    ...
    let r = Matrix3::from_rows([[c[0], c[1], c[2]], [c[4], c[5], c[6]], [c[8], c[9], c[10]]]);
    let t = Vector3::new([c[3], c[7], c[11]]);

    let s = Matrix3::from_rows([
        [1.0 + dsx, 0.0, 0.0],
        [0.0, 1.0 + dsy, 0.0],
        [0.0, 0.0, 1.0 + dsz],
    ]);

    let rx = Matrix3::from_rows([
        [1.0, 0.0, 0.0],
        [0.0, dtheta_x.cos(), -dtheta_x.sin()],
        [0.0, dtheta_x.sin(), dtheta_x.cos()],
    ]);
    let ry = Matrix3::from_rows([
        [dtheta_y.cos(), 0.0, dtheta_y.sin()],
        [0.0, 1.0, 0.0],
        [-dtheta_y.sin(), 0.0, dtheta_y.cos()],
    ]);
    let rz = Matrix3::from_rows([
