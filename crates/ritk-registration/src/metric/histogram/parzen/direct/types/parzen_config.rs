...
/// Precomputed Parzen-window parameters for one axis (SRP/SSOT).
///
/// Groups ÏƒÂ² and derived `half_width`/`inv_2sigma_sq`. `SampleWindow` takes
...
/// can produce ÏƒÂ²>10â¹ (near-equal range), yielding `half_width`>65535.
...
pub(crate) struct ParzenConfig {
    /// Parzen ÏƒÂ² in bin-index units.
    sigma_sq: f32,
    /// Half-width in bins (`usize`, reverted MEM-328-03 â€” see struct doc).
    half_width: usize,
    /// Precomputed `-0.5 / sigma_sq`.
    inv_2sigma_sq: f32 }

impl ParzenConfig {
    /// Return ÏƒÂ² in bin-index units.
    ///
    ...
    /// Construct from ÏƒÂ² in bin-index units.
    ///
    ...
    ///
    /// Converts via `sigma_sq=(sigma/bin_width)Â²`, then delegates to [`new`](Self::new).
    ...
    ///
    /// Encapsulates `floor â†’ BinRange::new` (was inlined at multiple sites).
    ///
    /// # Arguments
    /// * `val` â€” Normalised intensity in `[0, num_bins-1]`
    /// * `num_bins` â€” Histogram bins per axis
    #[inline]
    ...
    ///
    /// Encapsulates `bin_range â†’ StackWeights::new` (was inlined).
    ///
    /// # Arguments
    /// * `val` â€” Normalised intensity in `[0, num_bins-1]`
    /// * `num_bins` â€” Histogram bins per axis
    ...
    ///
    /// Returns `(range, weights, inv_sum)` where `inv_sum = 1/Î£ weights[j]`.
    ...
    ///
    /// Returns `Î£ exp(-(val-bin)Â²Ã—inv_2sigma_sq)`. For interior values
    /// approximates `âˆš(2Ï€ÏƒÂ²)`. Useful for cross-validating exp-ratchet.
    ///
    /// # Arguments
    /// * `val` â€” Normalised intensity in `[0, num_bins-1]`
    /// * `num_bins` â€” Histogram bins per axis
    ...
    /// `1/sum_weights` â€” normalization factor (PERF-328-01).
    ///
    /// Computes weights once. Approximates `1/âˆš(2Ï€ÏƒÂ²)` for interior values;
