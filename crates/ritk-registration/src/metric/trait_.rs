...
pub trait Metric<B: Backend, const D: usize> {
    ...
    /// across calls — e.g. the Parzen weight matrix `W_fixed^T [num_bins, N]`
    ...
    ///
    /// # Performance note
    /// For a 256³ volume with Mattes MI (50 bins), the full-grid `W_fixed^T`
    ...
    /// See `docs/audit_optimization_sprint_350.md` §2.3 for the breakdown.
