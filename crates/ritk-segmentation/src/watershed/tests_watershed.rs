...
fn test_constant_image_single_basin() {
    ...
    let result = WatershedSegmentation::new()
        .apply(&image)
...
fn plateau_flooding_matches_simpleitk_oracle_exactly() {
    ...
    let labels = WatershedSegmentation::new()
        .apply(&image)
        .expect("infallible: validated precondition");
    assert_eq!(
        labels
...
fn signed_zero_substitution_preserves_plateau_partition() {
    ...
    let positive_labels = WatershedSegmentation::new()
        .apply(&positive)
        .expect("infallible: validated precondition");
    let signed_labels = WatershedSegmentation::new()
        .apply(&signed)
        .expect("infallible: validated precondition");
    assert_eq!(
        signed_labels
...
fn plateau_partition_is_reversal_invariant() {
    ...
    let forward_labels = WatershedSegmentation::new()
        .apply(&forward)
        .expect("infallible: validated precondition");
    let reverse_labels = WatershedSegmentation::new()
        .apply(&reverse)
        .expect("infallible: validated precondition");
