...
#[test]
fn native_signed_twelve_bit_samples_use_bits_stored_sign_bit() {
    let bytes = [0x0fffu16, 0x0800, 0x07ff]
        .iter()
    ...
    assert_eq!(out, vec![-1.0, -2048.0, 2047.0]);
}
...
fn pixel_signedness_try_from_rejects_invalid_u16() {
    assert_eq!(
        PixelSignedness::try_from(0u16).expect("zero is unsigned DICOM pixel representation"),
        PixelSignedness::Unsigned
    );
    assert_eq!(
        PixelSignedness::try_from(1u16).expect("one is signed DICOM pixel representation"),
        PixelSignedness::Signed
    );
    assert_eq!(
        PixelSignedness::try_from(2u16).unwrap_err().to_string(),
        "pixel_representation=2 is invalid; expected 0 or 1"
    );
    assert_eq!(
        PixelSignedness::try_from(255u16).unwrap_err().to_string(),
        "pixel_representation=255 is invalid; expected 0 or 1"
    );
}
