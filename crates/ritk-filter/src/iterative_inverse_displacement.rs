...
impl IterativeInverseDisplacementField {
    /// Run the coordinate-descent search on host buffers, independent of how
    /// they were extracted and how the result is rebuilt.
    ...
    pub fn apply<B: Backend>(
        ...
        let geometry = dx.grid_geometry()?;
        let [rx, ry, rz] = self.invert_components(&ux, &uy, &uz, dims, &geometry);
        Ok((
            rebuild(rx, dims, dx),
            rebuild(ry, dims, dy),
            rebuild(rz, dims, dz),
