//! ITK-faithful sparse-field layer lists.
//!
//! Both SparseField filters (`canny_segmentation_level_set` and
//! `anti_alias_binary`) keep the same structure ITK does: `2·NL+1` doubly
//! linked lists of voxel indices, with per-voxel status in a parallel array.
//! Relinking a voxel — removing it from its current layer and pushing it to
//! the front of another — is the inner loop's most frequent mutation, and it
//! was quadratic: each `move_to` scanned its old layer for the voxel
//! (`VecDeque::iter().position()` then `remove`), which is `O(layer)` per
//! call.
//!
//! This module owns that structure once. It is an index-based linked list
//! (`prev`/`next`/`layer` per *member*, one `handle` slot per voxel), so:
//!
//! - **Relink is `O(1)`** — unlink through the handle, push at the front.
//! - **Order is preserved exactly.** Array-backed alternatives could remove
//!   in `O(1)` only by `swap_remove`, which reorders the layer; the active
//!   layer's iteration order is load-bearing in these filters (the update
//!   sweep writes to neighbouring layers' `phi`, and the ITK differential
//!   oracles pin the float-exact result), so an order-changing structure is
//!   not a valid substitute.
//! - **Memory stays band-proportional.** Nodes live in one slab with a free
//!   list, so the cost is `4 bytes per voxel` (the handle array, all voxels)
//!   plus `16 bytes per band member` — never a per-voxel node for the whole
//!   volume.
//!
//! Layer membership is tracked by the handle, not by `status`: the filters
//! deliberately leave a voxel linked in the active layer for the rest of the
//! sweep after marking it `ST_CUP`/`ST_CDN`, and rebuild the layer at the end
//! of the sweep. Deriving membership from `status` would unlink a voxel the
//! old code kept linked.
...
///
/// This is ITK's `ChangeLayer`/relink step, shared by both SparseField
/// filters. The membership test is the list's own handle, so the "voxel was
/// never in the layer its status names" case is a no-op exactly as the
/// layer scan was.
#[inline]
