...
/// Speckle (multiplicative) noise filter.
///
/// Applies multiplicative Gamma-distributed noise (mean 1, variance `ÏƒÂ²`):
///
/// ```text
/// I'(x) = I(x) Â· Î“(x),   E[Î“] = 1,  Var[Î“] = ÏƒÂ²
/// ```
///
/// Matches `sitk.SpeckleNoise` (run single-threaded): the Gamma variate is
/// generated by Marsagliaâ€“Tsang acceptanceâ€“rejection over ITK's MT19937 uniform
...
impl SpeckleNoiseFilter {
    ...
    fn apply_values(&self, values: &[f32]) -> Vec<f32> {
        // Degenerate variance â‡’ Gamma collapses to 1 (identity).
        if self.std == 0.0 {
        ...
        values
            .iter()
            .map(|&v| {
                // Marsagliaâ€“Tsang gamma for the fractional shape `delta`.
                let (mut xi, mut nu);
                loop {
