...
fn output_shape_unchanged_after_shift() {
    ...
    let shifted = FftShiftFilter::new()
        .apply(&img)
...
fn double_shift_is_identity() {
    ...
    let once = FftShiftFilter::new()
        .apply(&img)
        .expect("infallible: validated precondition");
    let twice = FftShiftFilter::new()
        .apply(&once)
        .expect("infallible: validated precondition");

    let (result, _) =
        ritk_tensor_ops::extract_vec(&twice).expect("infallible: validated precondition");
    let max_diff = result
        .iter()
        .zip(vals.iter())
        .map(|(a, b)| (a - b).abs())
...
fn dc_moves_to_center_after_shift() {
    ...
    let shifted = FftShiftFilter::new()
        ...
        .expect("infallible: validated precondition");
    let (shifted_data, _) =
        ritk_tensor_ops::extract_vec(&shifted).expect("infallible: validated precondition");
...
fn real_fft_shift_even_dims_is_identity_twice() {
    ...
    let once = RealFftShiftFilter::new()
        .apply(&img)
        .expect("infallible: validated precondition");
    let twice = RealFftShiftFilter::new()
        .apply(&once)
        .expect("infallible: validated precondition");
