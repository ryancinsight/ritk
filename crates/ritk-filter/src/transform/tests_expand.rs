...
fn expand_x_factor_two_matches_itk_grid() {
    let img = ts::make_image::<B, 3>(vec![0.0, 10.0, 20.0, 30.0], [1, 1, 4]);
...
fn expand_factor_one_is_identity() {
    let img = ts::make_image::<B, 3>((0..24).map(|i| i as f32).collect(), [2, 3, 4]);
...
fn expand_updates_spacing_and_origin() {
    ...
    let td = burn::tensor::TensorData::new(vec![0.0_f32, 4.0], burn::tensor::Shape::new([1, 1, 2]));
