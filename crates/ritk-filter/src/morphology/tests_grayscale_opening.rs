...
fn constant_image_unchanged() {
    ...
    let out = GrayscaleOpeningFilter::new(2)
        .apply(&img)
...
fn radius_zero_is_identity() {
    ...
    let out = GrayscaleOpeningFilter::new(0)
        .apply(&img)
        .expect("infallible: validated precondition");
...
fn bright_spike_removed() {
    ...
    let out = GrayscaleOpeningFilter::new(1)
        .apply(&img)
        .expect("infallible: validated precondition");
    let out_vals = extract_vals(&out);
...
fn idempotence() {
    ...
    let once = GrayscaleOpeningFilter::new(1)
        .apply(&img)
        .expect("infallible: validated precondition");
    let twice = GrayscaleOpeningFilter::new(1)
        .apply(&once)
        .expect("infallible: validated precondition");
