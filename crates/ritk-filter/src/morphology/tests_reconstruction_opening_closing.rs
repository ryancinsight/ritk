...
fn opening_by_reconstruction_removes_spike_keeps_plateau() {
    ...
    let out = OpeningByReconstructionFilter::new(1)
        .apply(&f)
...
fn closing_by_reconstruction_fills_pit_keeps_plateau() {
    ...
    let out = ClosingByReconstructionFilter::new(1)
        .apply(&f)
        .expect("infallible: validated precondition");
...
fn radius_zero_is_identity() {
    ...
    let obr = OpeningByReconstructionFilter::new(0)
        .apply(&f)
        .expect("infallible: validated precondition");
    let cbr = ClosingByReconstructionFilter::new(0)
        .apply(&f)
        .expect("infallible: validated precondition");
