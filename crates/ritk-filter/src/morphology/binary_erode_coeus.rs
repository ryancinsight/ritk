//! Coeus-native binary erosion.
//!
//! Atlas migration target: `super::binary_erode::erode_binary_3d` (already
//! substrate-agnostic — pure `&[f32]` in, `Vec<f32>` out, Moirai-parallel,
//! no Burn dependency) is the same core the Burn-generic
//! [`super::binary_erode::BinaryErodeFilter::apply`] calls. This module adds
//! a thin Coeus-`Image` boundary around that same core via
//! `crate::coeus_support::map_flat_image`, following the
//! `distance_transform_coeus` template: production Burn API stays, a
//! Coeus-native equivalent is added and verified against it, no algorithm
...
#[cfg(test)]
#[path = "tests_binary_erode_coeus.rs"]
mod tests;
