...
fn subtract_filter_computes_elementwise_difference() {
    ...
    let out = SubtractImageFilter::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
    let v = voxels(&out);
...
fn multiply_filter_computes_elementwise_product() {
    ...
    let out = MultiplyImageFilter::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
    let v = voxels(&out);
...
fn generic_binary_op_filter_matches_specialized() {
    ...
    let add_out = BinaryOpFilter::<AddOp>::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
    let sub_out = BinaryOpFilter::<SubtractOp>::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
    let mul_out = BinaryOpFilter::<MultiplyOp>::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
    let div_out = BinaryOpFilter::<DivideOp>::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
    let min_out = BinaryOpFilter::<MinOp>::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
    let max_out = BinaryOpFilter::<MaxOp>::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
...
fn squared_difference_computes_squared_residual() {
    ...
    let out = SquaredDifferenceImageFilter::new()
        .apply(&a, &b)
        .expect("infallible: validated precondition");
    let v = voxels(&out);
    // (3-1)²=4, (10-4)²=36, (-2-1)²=9
...
fn comparison_filters_produce_binary_masks() {
    ...
    let eq = voxels(
        &EqualImageFilter::new()
            .apply(&a, &b)
            .expect("infallible: validated precondition"),
    );
    let ne = voxels(
        &NotEqualImageFilter::new()
            .apply(&a, &b)
            .expect("infallible: validated precondition"),
    );
    let gt = voxels(
        &GreaterImageFilter::new()
            .apply(&a, &b)
            .expect("infallible: validated precondition"),
    );
    let ge = voxels(
        &GreaterEqualImageFilter::new()
            .apply(&a, &b)
            .expect("infallible: validated precondition"),
    );
    let lt = voxels(
