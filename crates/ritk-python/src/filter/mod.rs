...
pub fn register(parent: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(reinitialize_level_set, &m)?)?;
    ...
    m.add_function(wrap_pyfunction!(
        crate::color::gradient_recursive_gaussian,
        &m
    )?)?;
    ...
    m.add_function(wrap_pyfunction!(
        crate::color::label_map_contour_overlay,
        &m
    )?)?;
    m.add_function(wrap_pyfunction!(
        crate::color::physical_point_image_source,
        &m
    )?)?;
    m.add_function(wrap_pyfunction!(
        crate::color::vector_index_selection_cast,
        &m
    )?)?;
    ...
    m.add_function(wrap_pyfunction!(dicom_orient, &m)?)?;
    ...
    m.add_function(wrap_pyfunction!(inverse_displacement_field, &m)?)?;
    ...
    m.add_function(wrap_pyfunction!(signed_maurer_distance_map, &m)?)?;
    ...
    m.add_function(wrap_pyfunction!(masked_fft_normalized_correlation, &m)?)?;
