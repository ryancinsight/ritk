...
use super::{
    ...
    params::{
        CompositeUniforms, LightBlock, MaterialUniforms, SceneUniforms, SsaoUniforms,
    },
};
...
pub(super) fn submit_mesh_async(
    ...
    ctx.queue.write_buffer(&cache.scene_buf,         0, cast_slice(&[scene]));
    ctx.queue.write_buffer(&cache.lights_buf,        0, cast_slice(&[lights]));
    ctx.queue.write_buffer(&cache.material_buf,      0, cast_slice(&[material]));
    ctx.queue.write_buffer(&cache.ssao_uniforms_buf, 0, cast_slice(&[ssao]));
    ctx.queue.write_buffer(&cache.comp_params_buf,   0, cast_slice(&[CompositeUniforms {
    ...
    let mut encoder = ctx.device.create_command_encoder(
        &wgpu::CommandEncoderDescriptor { label: Some("mesh_encoder") }
    );
    ...
    cache.staging_buf.slice(..).map_async(wgpu::MapMode::Read, move |result| {
        tx.send(result).ok();
    });
    rx
}
...
pub(super) fn collect_mesh_result(cache: &GpuMeshFrameCache) -> ColorImage {
    ...
    ColorImage { size: [cache.cols, cache.rows], pixels }
}
...
fn encode_geometry_passes(
    ...
    {
        let color0_view = cache.color_array_tex.create_view(&wgpu::TextureViewDescriptor {
            dimension: Some(wgpu::TextureViewDimension::D2),
            base_array_layer: 0,
            array_layer_count: Some(1),
            ..Default::default()
        });
        let nd_view = cache.normal_depth_tex.create_view(&wgpu::TextureViewDescriptor::default());
        ...
        let bg = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry { binding: 0, resource: cache.scene_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 1, resource: cache.lights_buf.as_entire_binding() },
                wgpu::BindGroupEntry { binding: 2, resource: cache.material_buf.as_entire_binding() },
            ],
        });
    ...
    for i in 1..layers {
        let color_view = cache.color_array_tex.create_view(&wgpu::TextureViewDescriptor {
            dimension: Some(wgpu::TextureViewDimension::D2),
...
fn encode_ssao_pass(
    ...
    let bg = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        layout: &ctx.ssao_bgl,
        entries: &[
            wgpu::BindGroupEntry { binding: 0, resource: cache.ssao_uniforms_buf.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::TextureView(&nd_view) },
            wgpu::BindGroupEntry { binding: 2, resource: cache.ao_buf.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 3, resource: cache.ssao_kernel_buf.as_entire_binding() },
        ],
    });
...
fn encode_composite_pass(
    ...
    let color_array_view = cache.color_array_tex.create_view(&wgpu::TextureViewDescriptor {
    ...
    let bg = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&color_array_view) },
            wgpu::BindGroupEntry { binding: 1, resource: cache.ao_buf.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 2, resource: cache.output_buf.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 3, resource: cache.comp_params_buf.as_entire_binding() },
        ],
    });
