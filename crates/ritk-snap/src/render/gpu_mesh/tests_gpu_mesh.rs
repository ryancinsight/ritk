...
fn gpu_mesh_first_call_returns_none() {
    let Some(mut r) = GpuMeshRenderer::try_create() else {
        return;
    };
    if !gpu_available() {
        return;
    ...
    assert!(
        result.is_none(),
        "first call must return None before GPU completes"
    );
}
...
fn gpu_mesh_front_facing_triangle_produces_lit_pixels() {
    ...
    let img = r
        .render_sync(&mesh, &camera, &mat, &lights, 64, 64, &cfg)
        .expect("render_sync must return Some after blocking");

    let lit = img.pixels.iter().any(|px| px.a() > 0);
    assert!(
        lit,
        "front-facing triangle must produce at least one non-transparent pixel"
    );
}
...
fn gpu_mesh_output_dimensions_match_viewport() {
    ...
    let img = r
        .render_sync(&mesh, &camera, &mat, &lights, 80, 60, &cfg)
...
fn gpu_mesh_pixel_count_is_width_times_height() {
    ...
    let img = r
        .render_sync(&mesh, &camera, &mat, &lights, 32, 32, &cfg)
        .expect("render_sync must return Some");
...
fn gpu_mesh_empty_mesh_all_transparent() {
    ...
    assert!(
        result.is_none()
            || result
                .as_ref()
                .map(|img| img.pixels.iter().all(|px| px.a() == 0))
                .unwrap_or(false),
