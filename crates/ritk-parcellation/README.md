...
- **`freesurfer`** — the FreeSurfer surface family, read and written:
  triangle surfaces (`lh.white`, `lh.pial`), new-format morphometry (`curv`,
  `thickness`), annotations with their embedded colour table, ASCII labels, and
  the `FreeSurferColorLUT.txt` lookup table — plus cortical-ribbon
  rasterisation from an annotation into a volumetric parcellation.
