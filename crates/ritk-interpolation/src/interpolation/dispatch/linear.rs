...
/// only `Tensor<f32, B>` can implement this trait.
///
/// # See also
///
...
pub trait DispatchByShape<B: Backend>: sealed::Sealed {
    ...
    fn dispatch_by_shape(&self, indices: Tensor<f32, B>, mode: OutOfBoundsMode) -> Tensor<f32, B>;
}

impl<B: Backend> DispatchByShape<B> for Tensor<f32, B> {
    fn dispatch_by_shape(&self, indices: Tensor<f32, B>, mode: OutOfBoundsMode) -> Tensor<f32, B> {
...
// [`DispatchByShape`] trait method, which requires `&Tensor<f32, B>`. The
...
/// [`DispatchByShape`] trait method on `&Tensor<f32, B>`.
///
...
pub trait Dispatch3DTyped<B: Backend, const D: usize> {
    ...
    /// with a `match D` arm.
    ///
    /// The return type is always `Tensor<f32, B>` (the shape-routing always
    ...
    /// [`dispatch_linear`](super::dispatch_linear) (which returns `Tensor<f32, B>`) without a
    /// type mismatch.
    fn dispatch_3d_typed(&self, indices: Tensor<f32, B>, mode: OutOfBoundsMode) -> Tensor<f32, B>;
}
...
impl<B: Backend, const D: usize> Dispatch3DTyped<B, D> for Tensor<B, D> {
    fn dispatch_3d_typed(&self, indices: Tensor<f32, B>, mode: OutOfBoundsMode) -> Tensor<f32, B> {
        ...
        if D == 3 {
            // Narrowing: when D == 3, `self` is `&Tensor<f32, B>`. The sealed
            // `DispatchByShape` trait method requires `&Tensor<f32, B>`, so we
            // need to "narrow" the type. We do this via `clone().reshape(...)`,
            // which is safe and produces a `Tensor<f32, B>` that the trait
            // method can accept.
            //
...
/// sealed [`DispatchByShape`] trait method on `&Tensor<f32, B>`.
///
...
pub trait Dispatch1DTyped<B: Backend, const D: usize> {
    /// Narrow `self` to `&Tensor<f32, B>` and dispatch through the sealed
    /// [`DispatchByShape`] trait method.
    fn dispatch_1d_typed(&self, indices: Tensor<f32, B>, mode: OutOfBoundsMode) -> Tensor<f32, B>;
}
...
/// Extension trait that bridges `&Tensor<B, D>` (generic D) to the
/// sealed [`DispatchByShape`] trait method on `&Tensor<f32, B>`. See
/// [`Dispatch1DTyped`] for the full design rationale — this trait is
/// the D=2 equivalent.
pub trait Dispatch2DTyped<B: Backend, const D: usize> {
    /// Narrow `self` to `&Tensor<f32, B>` and dispatch through the sealed
    /// [`DispatchByShape`] trait method.
    fn dispatch_2d_typed(&self, indices: Tensor<f32, B>, mode: OutOfBoundsMode) -> Tensor<f32, B>;
}
...
pub trait Dispatch4DTyped<B: Backend, const D: usize> {
    /// Narrow `self` to `&Tensor<f32, B>` and dispatch through the sealed
    /// [`DispatchByShape`] trait method.
    fn dispatch_4d_typed(&self, indices: Tensor<f32, B>, mode: OutOfBoundsMode) -> Tensor<f32, B>;
}
