...
impl<B: Backend> VMambaBlock<B> {
    ...
    ///
    /// # Performance Notes
    /// This implementation minimizes tensor allocations by:
    /// 1. Reusing tensor shapes where possible
    /// 2. Avoiding redundant clones by careful ownership management
    /// 3. Computing intermediate values inline
    pub fn forward(&self, input: Tensor<B, 5>) -> Tensor<B, 5> {
        let dims = input.dims();
        let [batch, channels, depth, height, width] = dims;
        let _device = input.device();

        // First branch: SSM with cross-scan
        // LayerNorm expects channels last: [B, C, D, H, W] -> [B, D, H, W, C]
        // Clone input for residual connection later
        let input_permuted = input.clone().permute([0, 2, 3, 4, 1]);
        let x_norm1 = self.norm1.forward(input_permuted);
        ...
        // Apply SSM to each scanned direction with pre-allocated capacity
        let num_directions = scanned_sequences.len();
        let mut processed_sequences: Vec<Tensor<B, 3>> = Vec::with_capacity(num_directions);

        for seq in scanned_sequences {
            // seq shape: [batch, channels, seq_len]
            // Reshape to [batch*seq_len, channels] for SSM
            // Process inline to avoid closure overhead
            let [b, c, s] = seq.dims();
            let seq_flat = seq.permute([0, 2, 1]).reshape([b * s, c]);

            // Apply SSM and reshape back
            let processed = self.ssm.forward(seq_flat);
            let reshaped = processed.reshape([b, s, c]).permute([0, 2, 1]);
            processed_sequences.push(reshaped);
        }

        // Cross-merge: combine all directions
        let ssm_out = self.merge_directions(processed_sequences, dims, directions);

        // First residual connection - input is consumed here
        let x = input + ssm_out;

        // Second branch: FFN
        // LayerNorm expects channels last
        // Clone x for residual connection
        let x_permuted = x.clone().permute([0, 2, 3, 4, 1]);
        let x_norm2 = self.norm2.forward(x_permuted);
        let x_norm2 = x_norm2.permute([0, 4, 1, 2, 3]);

        // Flatten spatial for FFN: [B, C, D, H, W] -> [B*D*H*W, C]
        let x_flat = x_norm2
            .permute([0, 2, 3, 4, 1])
            .reshape([batch * depth * height * width, channels]);

        // FFN: expand -> activate -> project
        // Process inline to minimize temporaries
        let x_ffn = self
            .ffn_project
            .forward(self.act.forward(self.ffn_expand.forward(x_flat)));

        // Reshape back: [B*D*H*W, C] -> [B, C, D, H, W]
        let x_ffn_reshaped = x_ffn
            .reshape([batch, depth, height, width, channels])
            .permute([0, 4, 1, 2, 3]);

        // Second residual connection - x is consumed here
        x + x_ffn_reshaped
    }
    ...
    fn merge_directions(
        ...
        if self.cross_scan.use_3d() {
            self.cross_scan
                .merge_3d(sequences, depth, height, width, directions)
        } else {
