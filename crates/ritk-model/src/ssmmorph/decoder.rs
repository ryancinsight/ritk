...
impl SSMMorphDecoderConfig {
    ...
    pub fn stage_configs(&self, encoder_channels: &[usize]) -> Vec<DecoderStageConfig> {
        ...
        // Iterate reversed to match skip connections
...
impl<B: Backend> DecoderStage<B> {
    ...
    fn new(
        ...
        let upsample_config = ConvTranspose3dConfig::new([in_channels, out_channels], [4, 4, 4])
            .with_stride([2, 2, 2])
            .with_padding([1, 1, 1])
            .with_bias(false);
        ...
        let fusion_config = Conv3dConfig::new([fusion_in_ch, out_channels], [3, 3, 3])
            .with_stride([1, 1, 1])
...
impl<B: Backend> SSMMorphDecoder<B> {
    ...
    pub fn new(
        ...
        let stages: Vec<_> = stage_configs
            ...
            .enumerate()
            .map(|(i, stage_config)| {
                let skip_ch = if i < encoder_channels.len() {
                    encoder_channels[encoder_channels.len() - 1 - i]
                } else {
                    0
                };
        ...
        // Final projection mapping from decoder final layer to out_channels
        let final_ch = encoder_channels.first().copied().unwrap_or(16);
        let proj_config = Conv3dConfig::new([final_ch, config.out_channels], [3, 3, 3])
            .with_padding(PaddingConfig3d::Explicit(1, 1, 1))
            .with_bias(true);
...
mod tests {
    ...
    fn test_decoder_forward() {
        ...
        let skip1 = Tensor::<NdArray, 5>::zeros([1, 32, 16, 32, 32], &device);
        let skip2 = Tensor::<NdArray, 5>::zeros([1, 64, 8, 16, 16], &device);
        let skip3 = Tensor::<NdArray, 5>::zeros([1, 128, 4, 8, 8], &device);
        let skip4 = Tensor::<NdArray, 5>::zeros([1, 256, 2, 4, 4], &device);
        let skip_features = vec![skip1, skip2, skip3, skip4];
