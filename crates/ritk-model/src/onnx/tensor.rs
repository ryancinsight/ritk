...
pub fn burn_tensor_to_onnx<B: Backend, const D: usize>(
    ...
    // Reinterpret Vec<f32> allocation as Vec<u8> in-place (zero-copy) safely.
    onnx_tensor.raw_data = bytemuck::cast_vec(values);
...
mod tests {
    ...
    fn test_num_elements() {
        assert_eq!(shape_utils::num_elements(&[1, 3, 224, 224]), 3 * 224 * 224);
