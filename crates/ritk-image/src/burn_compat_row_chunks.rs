//! WGPU dispatch-limit chunked-row-apply helper for Burn tensors.
//!
...
//! route through [`apply_row_chunks`].
//!
//! This module is only available when the `burn-compat` feature is enabled,
//! because it operates on the legacy Burn-backed [`Tensor`](crate::tensor::Tensor).
