...
fn test_filter_cpr_creates_output() {
    ...
    let mut args = default_args(input, output.clone(), "cpr");
    args.cpr_points = vec!["0,0,0".to_string(), "4,4,4".to_string()];
    args.cpr_path_samples = 32;
    args.cpr_cross_samples = 16;
    args.cpr_half_width = 2.0;
...
fn test_filter_cpr_insufficient_points_errors() {
    ...
    let mut args = default_args(input, output, "cpr");
    args.cpr_points = vec!["0,0,0".to_string()];
...
fn test_filter_cpr_malformed_point_errors() {
    ...
    let mut args = default_args(input, output, "cpr");
    args.cpr_points = vec!["0,0".to_string(), "1,1,1".to_string()];
