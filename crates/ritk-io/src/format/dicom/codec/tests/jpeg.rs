...
fn test_decode_compressed_frame_rescale_contract() {
    ...
    let base = decode_compressed_frame(&obj, 0, 8, ritk_dicom::PixelSignedness::Unsigned, 1.0, 0.0)
        .expect("identity rescale decode");
    let scaled =
        decode_compressed_frame(&obj, 0, 8, ritk_dicom::PixelSignedness::Unsigned, 2.0, 10.0)
            .expect("slope=2 intercept=10 decode");
