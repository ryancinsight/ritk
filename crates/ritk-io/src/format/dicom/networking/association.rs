...
impl Association {
                ...
                for pc in &ac.presentation_contexts {
                    if pc.result_reason == 0 {
                        assoc.negotiated_contexts.push(NegotiatedContext {
                            ...
                            abstract_syntax_uid: rq_map.get(&pc.presentation_context_id)
                                .cloned().unwrap_or_default(),
                            transfer_syntax_uid: pc.transfer_syntax_uid.clone(),
                        });
                    }
                }
                assoc.remote_max_pdu_length = ac.user_information.maximum_length.maximum_length_received;
                assoc.active = true;
                Ok(assoc)
            }
    ...
    pub fn c_echo(&mut self) -> Result<u16> {
        let ctx = self.context_for_sop_class(sop_class::VERIFICATION, &[transfer_syntax::IMPLICIT_VR_LE])?;
    ...
    pub fn c_find(&mut self, sop_class_uid: &str, identifier: Vec<u8>) -> Result<FindResult> {
        ...
        loop {
            ...
            if s == DimseStatus::Pending as u16 || s == DimseStatus::PendingWarning as u16 {
                if let Some(ref d) = rsp.data_set { matches.push(d.clone()); }
            } else {
                final_status = s;
                break;
            }
    ...
    pub fn c_store(&mut self, sop_class_uid: &str, sop_instance_uid: &str, data_set: Vec<u8>) -> Result<u16> {
        let ctx = self.context_for_sop_class(sop_class_uid,
            &[transfer_syntax::IMPLICIT_VR_LE, transfer_syntax::EXPLICIT_VR_LE])?;
        let msg = DimseMessage::c_store_rq(
            self.next_message_id(), sop_class_uid, sop_instance_uid, 0x0000, data_set);
        self.send_message(ctx, &msg)?;
        let (_, rsp) = self.recv_message()?;
    ...
    pub fn c_move(&mut self, sop_class_uid: &str, move_destination: &str, identifier: Vec<u8>) -> Result<MoveResult> {
        ...
        let msg = DimseMessage::c_move_rq(
            self.next_message_id(), sop_class_uid, move_destination, identifier);
        self.send_message(ctx, &msg)?;
        ...
        loop {
            ...
            if let Some(v) = le((0x0000,0x1023)) { warn = v; }
            if let Some(v) = le((0x0000,0x1020)) { rem = v; }
            if s != DimseStatus::Pending as u16 && s != DimseStatus::PendingWarning as u16 {
                final_status = s;
                break;
            }
        }
        Ok(MoveResult { completed: comp, failed: fail, warning: warn, remaining: rem, status: final_status })
    }
    ...
    pub fn abort(mut self) -> Result<()> {
        if !self.active { return Ok(()); }
        let _ = self.send_pdu(&Pdu::Abort(AbortPdu { source: AbortSource::DicomUlServiceUser }));
        self.active = false;
        Ok(())
    }
    ...
    fn send_pdu(&mut self, pdu: &Pdu) -> Result<()> {
        self.stream.write_all(&pdu.encode()).with_context(|| "PDU write")?;
        self.stream.flush()?;
        Ok(())
    }
    ...
    fn recv_pdu(&mut self) -> Result<Pdu> {
        let mut hdr = [0u8; 6];
        self.stream.read_exact(&mut hdr).with_context(|| "PDU header read")?;
        let len = u32::from_be_bytes([hdr[2], hdr[3], hdr[4], hdr[5]]) as usize;
        let mut body = vec![0u8; len];
        self.stream.read_exact(&mut body).with_context(|| "PDU body read")?;
    ...
    fn send_message(&mut self, ctx_id: u8, msg: &DimseMessage) -> Result<()> {
        let max = (self.remote_max_pdu_length.saturating_sub(6) as usize)
            .max(DEFAULT_MAXIMUM_LENGTH as usize - 6).max(1);
    ...
    fn fragment_and_send(&mut self, cid: u8, data: &[u8], ct: CommandType, max: usize) -> Result<()> {
        let mk_pdv = |is_last: bool, d: &[u8]| PresentationDataValueItem {
            presentation_context_id: cid,
            message_control_header: MessageControlHeader { message_type: ct, last_fragment: is_last },
            data: d.to_vec(),
        };
        if data.is_empty() {
                    ...
                    for pdv in &pd.presentation_data_value_items {
                        ...
                        match pdv.message_control_header.message_type {
                            CommandType::Command => {
                                ...
                                if pdv.message_control_header.last_fragment { cmd_last = true; }
                            }
    ...
    fn find_context(&self, uid: &str) -> Option<&NegotiatedContext> {
        self.negotiated_contexts.iter().find(|c| c.abstract_syntax_uid == uid)
    }

    fn context_for_sop_class(&mut self, uid: &str, _ts: &[&str]) -> Result<u8> {
        self.find_context(uid).map(|c| c.presentation_context_id)
    ...
    pub fn build_associate_rq(config: &AssociationConfig) -> Pdu {
        let mut nid: u8 = 1;
        let pcs: Vec<_> = config.presentation_contexts.iter().map(|rpc| {
            let mut ts = rpc.transfer_syntax_uids.clone();
            if !ts.iter().any(|t| t == transfer_syntax::IMPLICIT_VR_LE) {
                ts.push(transfer_syntax::IMPLICIT_VR_LE.to_string());
            }
    ...
    pub fn fragment_pdvs(data: &[u8], ct: CommandType, max: usize) -> Vec<PresentationDataValueItem> {
        let cid = 1u8;
        if data.is_empty() {
            return vec![PresentationDataValueItem {
                presentation_context_id: cid,
                message_control_header: MessageControlHeader { message_type: ct, last_fragment: true },
                data: Vec::new()
            }];
        }
        let chunks: Vec<&[u8]> = data.chunks(max).collect();
        chunks.iter().enumerate().map(|(i, c)| PresentationDataValueItem {
            presentation_context_id: cid,
...
fn pdv_last_data(pd: &PDataTfPdu) -> bool {
    pd.presentation_data_value_items.iter()
        .any(|p| p.message_control_header.message_type == CommandType::DataSet
