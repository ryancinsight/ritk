...
fn loopback_config(port: u16) -> AssociationConfig {
    AssociationConfig::new(
        ...
        DicomAddress::new(
            "127.0.0.1",
            port,
            AeTitle::new("TEST_SCP").expect("infallible: validated precondition"),
        ),
    )
        ...
        loop {
            match assoc.receive() {
                Ok(Pdu::PData { data: pdv_list }) => {
                    ...
                    let cmd = parse_command_response(&cmd_bytes)
                        .expect("infallible: validated precondition");
...
fn c_move_loopback_returns_final_success_status() {
    ...
    let config = loopback_config(port);
    let dest =
        MoveDestination::new(AeTitle::new("MY_SCP").expect("infallible: validated precondition"));
