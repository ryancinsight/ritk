...
pub fn fod_peak_direction_field<'a>(
    ...
    let strongest: Option<Vector<3>> = peaks
        .first()
        .map(|peak| Vector::new(peak.direction));

    move |_point: &Point<3>| -> Option<Vector<3>> {
        strongest
    }
}
...
pub fn dti_pev_direction_field<'a>(
    ...
    let direction: Option<Vector<3>> =
        (!degenerate).then(|| Vector::new(pev));
...
pub fn fod_volume_direction_field<'a>(
    ...
    move |point: &Point<3>| -> Option<Vector<3>> {
        volume.direction_at(point, 50, 100, 0.1)
    }
}
