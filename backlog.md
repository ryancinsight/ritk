...
- Status: review; priority: P1; owner: RITK viewer + integration; integrator: root; branch: `build/ritk-metis-current-lock-021`; last-update: 2026-09-22; delivery: PR [#604](https://github.com/ryancinsight/ritk/pull/604).
...
- Dependencies: Atlas pins Metis `1b10541c2ef7a849e6ff66a3c778874bdf96de7b`; Atlas retains Moirai at the reachable clean pin `b77239dd10bcaf803394c26255c462bc858c1340` while its current default `29b129659f368fcbc3168103302cfc545411e642` carries the separate conformance waiver.
...
- Verification: dependency-free script tests, browser syntax/fixture checks, locked native/WASM package gates, and hosted responsive raster evidence. Hosted run [35724926751](https://github.com/ryancinsight/ritk/actions/runs/35724926751) passes Chromium and Firefox responsive jobs against RITK `67d4ad44` and Metis `776dbbf9`; artifacts [10693572248](https://github.com/ryancinsight/ritk/actions/runs/35724926751/artifacts/10693572248) and [10692907100](https://github.com/ryancinsight/ritk/actions/runs/35724926751/artifacts/10692907100) and exact trace/derivative hashes are in `docs/manual/images/dicom-metis-real-browser-mri-responsive.json`.
...
- Verification: locked `ritk-snap` nextest 495/495, native/WASM checks and strict Clippy, formatting, 4 doctests plus 1 intentional ignore, warning-clean Rustdoc, Python/browser script tests 28/28, and deterministic layout/property tests; native real-MRI responsive capture is 1280×800 RGBA, SHA-256 `056bf2cd8828df63972af2fe785e436ef0256e501a3ac7386535db0db169a0c9`, 534,414 non-black pixels; hosted run [35724926751](https://github.com/ryancinsight/ritk/actions/runs/35724926751) passes Chromium and Firefox responsive lanes against RITK `67d4ad44` and Metis `776dbbf9`; exact trace and derivative hashes are in `docs/manual/images/dicom-metis-real-browser-mri-responsive.json`.
...
- Status: done; priority: P1; owner: RITK viewer + integration; integrator: root; last-update: 2026-09-22; dependency: Metis PR #348 merge `c29d8a302c71a0b7a9590d55793968e3197ea019`; hosted source revision `776dbbf94593e42d0a5686b587ed27b72f885a73`.
...
- Risk: [patch]; this is a dependency/provenance advance only. Diagnostic dispatch `35703095989` correctly exposed the old-lock exact-revision guard; merged-main hosted dispatch `35724926751` is the current evidence record and retains explicit WebKit and WebGPU capability residuals.
- Verification: standalone lock check passes with 63 first-party Git sources at SHA-256 `602cba1b0a1a2b6ce8bdb61f42d9c3ef2844bdfd76e5120a29b5a6d8c46552ec`; locked `ritk-snap` nextest passes 495/495 (run `d6fb7041-ad04-4646-a03c-7c7e6c15e414`); strict native Clippy, WASM check/Clippy, formatting, warning-clean Rustdoc and doctests (4 passed, 1 ignored) pass. The standalone replay reads 94 `.dcm` files/49,807,236 bytes, exits 0, rejects the invalid-study probe with exit 1, and preserves the 1280×800 real-MRI image (`259dd79103482756c4e688621bebafc841cc40f1df10ff2bbd7f9d04b7b4d401`, 411,589 non-black pixels); current executable SHA is `14412a80c37a19a2f4a09cb08165831a2311d4476f24d1ee159b34406eaf9b30`, example SHA `47eeb88b1ccf8720f6c8e8c8b05b6e7a19a4922d4afdbe7677067333028bd6b3`.
- Delivery: lock/workflow commit `c842689b985beecb159bba17cc1b3a6a50e67c6a` (RITK PR #594) advances all six Metis package sources and both workflow revision references to `776dbbf94593e42d0a5686b587ed27b72f885a73`; hosted run [35724926751](https://github.com/ryancinsight/ritk/actions/runs/35724926751) accepts the 94-file study on Chromium, Firefox, Chromium-window, responsive and MIP lanes. Safari 26.6.2 accepts the chooser but rejects the bounded whole-file read; Chromium has no WebGPU adapter. Exact artifacts, traces, failure captures and derivatives are in `docs/manual/images/dicom-metis-real-browser-mri-cross-engine.json`, `dicom-metis-real-browser-mri-responsive.json` and `dicom-metis-real-browser-mri-tools.json`.
...
- Delivery: PR [#588](https://github.com/ryancinsight/ritk/pull/588), merge `27ac7d12bc69a04f7fb12e5ba41ef8fd4fe93aa9`; source commit `b671ae72a20bb6d113de35314e73874f197d2a83`, provenance commit `ffcc7b835b5890b4cdfe87451b1f56f11d9c4738`.
...
- Verification: merged-main hosted run [35704771969](https://github.com/ryancinsight/ritk/actions/runs/35704771969) passes Chromium raster, Chromium window, Chromium MIP projection and Firefox on the 94-file public MRI-DIR study. The Chromium-window artifact [10683893322](https://github.com/ryancinsight/ritk/actions/runs/35704771969/artifacts/10683893322) records Length `102.75155`, Angle `3.5569937`, ROI Rect `5402.25`, ROI Ellipse `4146.0703` and HU Point `27.0`, seven rejected invalid probes, and listener cleanup from 21/34 mounted to 0/0 stopped. The Chromium raster artifact [10684785508](https://github.com/ryancinsight/ritk/actions/runs/35704771969/artifacts/10684785508), Firefox artifact [10683538718](https://github.com/ryancinsight/ritk/actions/runs/35704771969/artifacts/10683538718) and projection artifact [10684320687](https://github.com/ryancinsight/ritk/actions/runs/35704771969/artifacts/10684320687) bind exact RGBA/semantic evidence; WebKit artifact [10683583337](https://github.com/ryancinsight/ritk/actions/runs/35704771969/artifacts/10683583337) and WebGPU artifact [10683659854](https://github.com/ryancinsight/ritk/actions/runs/35704771969/artifacts/10683659854) remain explicit host-capability residuals.
...
- Evidence: current hosted [run 35724926751](https://github.com/ryancinsight/ritk/actions/runs/35724926751) builds RITK `67d4ad4457823f928b02739ec120bc0329a1b7f0` against Metis `776dbbf94593e42d0a5686b587ed27b72f885a73`; Chromium and Firefox pass the 94-file real MRI study, three exact pixel oracles and lifecycle checks; WebKit accepts the chooser but denies the first bounded read under Safari 26.6.2, with diagnostics artifact [10692622232](https://github.com/ryancinsight/ritk/actions/runs/35724926751/artifacts/10692622232) preserved in the provenance JSON.
...
- Blocker: exact SafariDriver/WebKit selected-file authorization defect remains external; current run 35724926751 reproduces the denial after SafariDriver accepts all 94 files (artifact [10692527187](https://github.com/ryancinsight/ritk/actions/runs/35724926751/artifacts/10692527187), diagnostics [10692622232](https://github.com/ryancinsight/ritk/actions/runs/35724926751/artifacts/10692622232)). Re-open when the corrected browser/runner path grants real-file reads; application byte-read APIs cannot grant that access.
...
- Blocker: hosted Chromium in run [35724926751](https://github.com/ryancinsight/ritk/actions/runs/35724926751) reports no WebGPU adapter; the setup error and failure capture artifact [10692602261](https://github.com/ryancinsight/ritk/actions/runs/35724926751/artifacts/10692602261) are preserved in [`dicom-metis-real-browser-mri-webgpu-failure.png`](docs/manual/images/dicom-metis-real-browser-mri-webgpu-failure.png).
