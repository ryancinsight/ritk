...
- Browser keyboard increment (2026-09-14, pending Metis PR #142): the browser canvas adapter maps bounded Métis keydown/keyup records to RITK's shared virtual-key contract, preserves key-repeat state for cine controls, ignores unsupported codes, and assigns `tabindex="0"` so each canvas owns keyboard focus. Native mapping tests and the locked 816-test `ritk-snap` suite pass; the WASM gate and hosted browser input evidence remain pending the merged Metis dependency.
