...
- **MIG-654-01 [patch] - Restore CI dependency topology (IN PROGRESS; owner=Codex; scope=GitHub workflows/dependency checkout/nextest policy/PyO3 feature boundary/Python test dependencies/registration objective performance/PM artifacts; last-update=2026-07-13).** Acceptance: every workflow that loads the workspace checks out the complete pinned sibling dependency graph at the paths declared by Cargo, Rust tests run under the committed 30-second slow/60-second termination policy and link outside extension-module mode, parity environments install their declared oracles, CMA-ES honors explicit parallel dispatch without repeated fixed-cache work, SyN avoids redundant local-correlation, integration, and interpolation work, and the fix-forward PR merges green.
