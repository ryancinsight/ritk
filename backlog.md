...
- Browser input increment (working branch `feat/ritk-snap-canvas-input-001`): the three Métis canvas presenters retain bounded pointer and wheel listeners, translate target-local coordinates and wheel units into RITK's shared `PresentationEvent` reducer, route orthogonal canvases by axis, and cancel lost gestures through `PointerCancel`. DICOM parsing, metadata, geometry, pixel presentation and viewer state remain RITK-owned. The locked consumer graph advances to Metis PR #64 at `58cf0259`; native and WASM checks pass. Decision: [ADR 0031](docs/adr/0031-bounded-browser-canvas-input.md).
