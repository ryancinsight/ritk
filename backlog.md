...
- Current increment (2026-09-14): the consumer validator now accepts an explicit keyboard mode that requires focused, trusted `ArrowDown` keydown/keyup evidence on every canvas, including key/code, repeat, modifiers and target checks. Keyboard traces add an `after-keyboard` semantic snapshot so wheel progression is compared with the state immediately before the wheel. The manual documents the `--require-keyboard` command and keeps keyboard records separate from DICOM pixel claims. The locked native suite passes 830/830 with strict native/WASM Clippy and checks; hosted cross-engine run 34895454734 is executing against the merged RITK main head.
- Dependency-lock increment (2026-09-14): the browser consumer lock and workflow defaults now resolve all six Metis packages to merged Metis `02d4047c5567834667ab9beb796ea27f6257f0ad`; the standalone lock preserves Moirai `c110452ec8a8057a98deab330f9047b1c7efd522` and hosted run 34895454734 is executing against merged RITK main.
...
- Browser keyboard increment (2026-09-14): [RITK PR #379](https://github.com/ryancinsight/ritk/pull/379), merge `e524e14f7`, consumes the merged Métis PR #142 at `9413ed27ce6dc547bc566c481f4d36a17bc97e35` with Moirai `c110452ec8a8057a98deab330f9047b1c7efd522`. The browser canvas adapter maps bounded keydown/keyup records to RITK's shared virtual-key contract, preserves key-repeat state for cine controls, ignores unsupported codes, and assigns `tabindex="0"` so each canvas owns keyboard focus. Native mapping tests, the locked 816-test `ritk-snap` suite, and the locked WASM check/Clippy gates pass; hosted run 34895454734 is executing with the real-study keyboard trace.
- Keyboard evidence continuation (2026-09-14): RITK now resolves all Métis packages at merged PR #143 commit `0c3444d1b37b8d16813fa45d825b91027b1b5b1b`; the hosted workflow default and lockfile agree. The Linux-only `CineTick` re-export lint is fixed in `d08651450`; locked `ritk-snap` nextest remains 816/816 with strict native and WASM checks. The hosted real-study workflow is ready to validate focus and trusted `ArrowDown` traces on Chromium, Firefox, and WebKit.
