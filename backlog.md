...
- Browser WebGPU evidence increment (2026-09-16, in-progress): the saved-study matrix now derives a dimensions-and-attributes oracle, runs Chromium with the RITK-owned `?renderer=webgpu` page query, and asks Metis for a named `webgpu` context plus real element PNGs. Raster entries keep the RGBA oracle; the WebGPU entry does not claim 2D equivalence or hardware acceleration. Metis PR #190 merged at `29812898870042665010c6534dce111ea1966925`; the standalone lock and workflow default now pin that revision, with Moirai at `95275651722583f52e098c0d30b1a53ec82c1fc7`. A hosted failure remains evidence of unavailable browser WebGPU, not a raster success.
- WebGPU diagnostic increment (2026-09-16): RITK's gallery now renders string-valued WASM setup errors instead of reading a missing `.message` property, preserving adapter/device diagnostics in `gallery-status`. The fix is on `fix/ritk-webgpu-error-diagnostic`; the hosted rerun remains the acceptance step for the actual browser capability.
