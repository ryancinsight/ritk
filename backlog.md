...
- Native picker increment (2026-09-14): Windows no-PATH selection of the saved public MRI-DIR T2 folder is verified through actual input injection. The interactive window displays all three RITK planes; pathless framebuffer capture matches explicit-path capture byte-for-byte (1280 × 800, 411,589 non-black pixels); both exit 0. Real dialog cancellation exits 1 before viewer creation. [Capture and provenance](docs/manual/images/dicom-metis-real-mri.json).
- Picker boundary evidence: four launch tests cover explicit-path bypass, folder forwarding, cancellation and typed provider-error propagation. Locked `ritk-snap` nextest passes 831/831 (run `322499d4-2581-4343-a2a5-2287894edca6`), strict all-target Clippy and workspace fmt pass, and the standalone lock resolves. Documentation gates are queued on the shared build cache. RITK retains all DICOM semantics; native OS permission-denial and unrelated host-migration acceptance remain open.
