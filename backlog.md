...
- Native picker increment (2026-09-14): RITK now permits `ritk-snap --metis-native` without a positional path on Windows. Metis delegates the bounded folder selection to Moirai; RITK retains DICOM discovery, series validation, decode, geometry, and clinical display. An explicit path and `--series-instance-uid` remain required for multi-series folders. Focused native gates and a real saved-study capture are required before this increment is closed.
