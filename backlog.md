...
- Action-adapter increment: `ViewerViewport` maps validated display placement into RITK image coordinates; the eframe shell now feeds its pointer response through the shared event dispatcher and applies actions to `SnapApp`. `ImagePoint` and `ViewportOffset` remove egui point carriers from interaction state. A correction preserves multi-click measurements, accumulates repaint requests, cancels lost-pointer gestures, and carries fractional display coordinates without rounding. Verification: locked `ritk-snap` nextest 773/773, strict Clippy, formatting and package checks pass; DICOM loaders and clinical state remain RITK-owned.
