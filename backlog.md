...
- Current increment (2026-09-14): the consumer validator now accepts an explicit keyboard mode that requires focused, trusted `ArrowDown` keydown/keyup evidence on every canvas, including key/code, repeat, modifiers and target checks. Keyboard traces add an `after-keyboard` semantic snapshot so wheel progression is compared with the state immediately before the wheel. The manual documents the `--require-keyboard` command and keeps keyboard records separate from DICOM pixel claims. Hosted run [34895454734](https://github.com/ryancinsight/ritk/actions/runs/34895454734) passes the keyboard and saved-study workflow on Chromium 152 and Firefox 155; Safari 26.6.2 accepts the chooser event but rejects the first bounded browser read before keyboard actions. The exact revisions, traces, artifact links and actual galleries are recorded in [`dicom-metis-real-browser-mri-cross-engine.json`](docs/manual/images/dicom-metis-real-browser-mri-cross-engine.json).
- Dependency-lock increment (2026-09-14): RITK PR #387 (`ea409f3c09a715b4f1b898b5738102a3832baf28`) resolves all six Metis packages to `befc5333ff4869e632eb38b14997ce865e4dfbe9` from Metis PR #150 and all fifteen Moirai packages to `27d9b06190e8e3db930ef9745187e0ea48cadfce` from Moirai PR #342; the standalone lock resolution, WASM check and strict Clippy, native strict all-target Clippy, and `ritk-snap` nextest gates pass. The workflow default will use this Metis revision for the hosted saved-study replay.
...
- Browser keyboard increment (2026-09-14): [RITK PR #379](https://github.com/ryancinsight/ritk/pull/379), merge `e524e14f7`, consumes the merged Métis PR #142 at `9413ed27ce6dc547bc566c481f4d36a17bc97e35` with Moirai `c110452ec8a8057a98deab330f9047b1c7efd522`. The browser canvas adapter maps bounded keydown/keyup records to RITK's shared virtual-key contract, preserves key-repeat state for cine controls, ignores unsupported codes, and assigns `tabindex="0"` so each canvas owns keyboard focus. Hosted run [34895454734](https://github.com/ryancinsight/ritk/actions/runs/34895454734) validates the real-study keyboard trace on Chromium 152 and Firefox 155; Safari's bounded read remains the provider residual.
- Keyboard evidence continuation (2026-09-14): hosted run [34895454734](https://github.com/ryancinsight/ritk/actions/runs/34895454734) validates focus and trusted `ArrowDown` traces on Chromium 152 and Firefox 155 for all three canvases. Safari 26.6.2 accepts the chooser event but rejects the first bounded browser read before keyboard actions; the exact failure artifact remains the WebKit provider regression case.
...
- Native picker increment (2026-09-14): integrator: root; scope: `crates/ritk-snap/src/launch.rs`, picker evidence in this item and `checklist.md`, `docs/manual/dicom-workflow.md`, and its public-study capture provenance. Acceptance: launch Windows `ritk-snap --metis-native` without PATH, select the saved public MRI-DIR T2 folder through the native dialog, inspect the RITK frame, verify the selection/error boundary and native package gates. RITK retains DICOM discovery, series validation, decode, geometry, and display; multi-series folders still require explicit PATH and `--series-instance-uid`.
- Native picker verification increment (2026-09-14): RITK's lock resolves Metis `e3d1cdbcaf4065030f61a2f2a75765a729c5e203` and Moirai `3746941f810d696ced5bd7de6f1eb391e2857207`. The Windows picker boundary tests cover explicit paths, selected folders, and cancellation; standalone `ritk-snap` check, strict all-target Clippy, 833/833 nextest, doctests, rustdoc, and the saved public CT workflow pass. `scripts/viewer.py` opened the real `test_data/3_head_ct_mridir/DICOM` series through Métis with its explicit UID, exited 0, rendered a 1280 × 800 application frame with 359,857 non-black pixels, and matched SHA-256 `8082cea87348126ce5a07cacb602bf71081747881481d2062bb1f2c7314113d3`. The physical folder-picker click path remains a manual desktop residual; no patient pixels or identifiers were committed.
