...
- Evidence: [run 34949979770](https://github.com/ryancinsight/ritk/actions/runs/34949979770), RITK `8a5698f0d`, Metis `e5379dd`, Moirai `8d032e38`: Chromium/Firefox pass 94 file hashes, three exact pixel oracles, cine-rate/repeat checks, rejections and cleanup. Safari 26.6.2 fails before RITK receives a batch.
...
- Verification: locked RITK nextest 847/847, strict native all-target Clippy, formatting and locked WASM check pass; [proof and log hashes](docs/manual/images/dicom-metis-real-browser-mri-cross-engine.json).
...
- Status: in-progress; priority: P1; owner: RITK viewer + Métis framework; integrator: root; regions: `Cargo.lock`, `.github/workflows/metis-browser-dicom.yml`, `docs/manual/dicom-workflow.md`, `docs/adr/0033-browser-trace-validator.md`, `docs/adr/0032-browser-semantic-snapshot.md`, `backlog.md`; last-update: 2026-09-15; branch: `build/ritk-moirai-size-lock`;
...
- Provider-lock increment (2026-09-15, branch `build/ritk-moirai-size-lock`): the standalone lock now resolves all six Metis packages to merged Metis PR #162, main `b374ca937ce6ddbcbce2fb55a0dd074a241ea956`, and all fifteen Moirai packages to merged PR #353, main `2451a3155c44dcf76d5577e4eb8c08badde51a0a`. The chooser workflow default is pinned to the same Metis revision. Local RITK package gates and the hosted saved-study replay remain required before this increment closes; DICOM parsing and image semantics remain RITK-owned.
- Browser cine-rate trace increment (2026-09-15): RITK commit `19b6f94c6` and the standalone lock/workflow update pin merged Metis `539fe42776c7eadee702238d22d610e8a1ec5d64`; native `ritk-snap` nextest passes 819/819, strict native Clippy, wasm32 check/Clippy and format pass. The hosted cross-engine cine-rate run is required before this increment closes; no new pixel claim is made yet.
