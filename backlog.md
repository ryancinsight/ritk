...
- Orthogonal browser increment (2026-09-11): the same Chromium workflow accepted the maximum 64-file bounded batch from the saved MRI-DIR series and produced reviewed axial, coronal and sagittal PNGs with machine-readable provenance. A 409-file single drop was rejected by Métis's explicit 64-file bound before RITK loading; complete browser studies therefore require a RITK-owned directory or multi-batch assembly contract. The native full-series path remains verified; this browser-ingress gap is open.
