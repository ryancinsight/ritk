...
- Wheel/modifier increment: RITK now translates Moirai modifier snapshots and wheel deltas into the shared presentation/action path; Ctrl/Command wheel uses the existing zoom policy and plain vertical wheel steps the active slice. Non-finite deltas and out-of-viewport events do not mutate viewer state. Métis remains transport/rendering only; DICOM parsing, decoded volume state, and clinical semantics remain in RITK. Focused action, native translation, adapter, formatting, check, and Clippy gates pass against the local provider overlay; standalone dependency lock refresh follows the merged Moirai and Métis revisions.
