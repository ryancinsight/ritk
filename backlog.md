...
- Browser chooser increment (2026-09-12): the packaged RITK viewer consumed all 409 real public CT files through Metis `#file-input` and Moirai's bounded `selected_files` source, read 216,156,416 bytes, and reported ready presented axial, coronal and sagittal frames at 512 × 512, 512 × 409 and 512 × 409 pixels. The live Chromium viewport showed non-black anatomy; the source revisions and exact DOM attributes are recorded in [the DICOM workflow manual](docs/manual/dicom-workflow.md#select-the-saved-study-through-the-browser-file-chooser). DICOM parsing, geometry, pixel values and viewer state remain RITK-owned; physical file-manager input, Firefox/WebKit and WebGPU remain open.
