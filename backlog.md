...
- Completed increment: `start_web_canvas` mounts the Métis browser host, routes its bounded named-byte drops through RITK's shared reducer, opens synthetic Part 10 DICOM bytes with the existing RITK loader, and presents one selected slice through Moirai's bounded canvas provider. `start_web_orthogonal_canvases` now presents axial, coronal and sagittal RITK frames through three named canvases from the same bounded handoff. Presentation tests assert axis order and dimensions; pointer actions, GPU upload and physical three-canvas capture remain open.
- WASM packaging increment: `ritk-snap` now builds as a `cdylib` library for `wasm32-unknown-unknown`; the lockfile resolves Mnemosyne at merged `be6efcb8`, and `wasm-bindgen 0.2.128 --target web` packages the artifact with `start_web` and `start_web_canvas` exports. The release build and package pass against the live Atlas overlay; the locked standalone/CI path is documented, and browser runtime visual capture remains open.
