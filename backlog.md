...
- Status: in-progress; priority: P1; owner: RITK viewer + Métis framework; integrator: root; branch: `build/ritk-moirai-safari-lock`; regions: Cargo.lock, .github/workflows/metis-browser-dicom.yml, backlog.md, docs/manual/dicom-workflow.md, docs/manual/images/dicom-metis-real-browser-mri-cross-engine.json; last-update: 2026-09-15;
...
- Dependency-lock increment (2026-09-15): all fifteen Moirai packages now resolve to merged PR #344 at `3ccfb1b76f386321aa24a86e65fe81efdbdfad84`, which adds the bounded direct `File.arrayBuffer()` read for Safari's first saved-file request. The six Metis packages remain at merged `305f24785b623a46b1962e73474a1a18dc6b3721` until the bounded consumer-read PR lands; the standalone lock, WASM, strict Clippy, native and `ritk-snap` gates remain required before the hosted replay.
