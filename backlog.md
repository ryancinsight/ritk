...
- Packaging increment: RITK now owns the root `metis.json`, a single-binary manifest consumed by Métis `package`; `Cargo.lock` resolves all Métis packages at `2e21146c6a9de73666396705c25dfa7555eb172c`. It carries no DICOM or patient-data resources; the resulting portable executable and per-user MSI launch the RITK-owned loader and viewer. The local Windows x64 package smoke exited 0, recorded the executable and MSI hashes in ADR 0026, and passed `ritk-snap.exe --help`; `.github/workflows/metis-package.yml` provides the pinned manual hosted artifact path. Registry publication and signing remain release-authority work.
...
- WASM packaging increment: `ritk-snap` now builds as a `cdylib` library for `wasm32-unknown-unknown`; the lockfile resolves Mnemosyne at merged `be6efcb8`, and `wasm-bindgen 0.2.128 --target web` packages the artifact with `start_web`, `start_web_canvas`, and `start_web_orthogonal_canvases` exports. The release build and package pass against the live Atlas overlay; the locked standalone/CI path is documented, and browser runtime visual capture remains open.
- Browser visual smoke increment: a local HTTP run of the packaged bundle dispatched the three synthetic `dicom_workflow` Part 10 files through a browser `DataTransfer`, observed three accepted 654-byte files and 1,962 bytes read, and inspected a non-black 4 × 2 RGBA frame displayed at 512 × 512 CSS pixels. The reviewed PNG is [`docs/manual/images/dicom-metis-browser.png`](docs/manual/images/dicom-metis-browser.png); the synthetic DOM event is untrusted, so physical drag-and-drop, browser-engine, pointer, three-view, GPU, and complete application-window capture remain open.
