...
- Branch: `refactor/ritk-conformance-ratchet-2026-09-13`; regions: `crates/ritk-snap/src/browser_trace.rs`, `crates/ritk-snap/src/browser_trace/validation.rs`, `crates/ritk-snap/src/browser_trace/tests.rs`, `crates/ritk-snap/src/render/gpu_volume/renderer.rs`, `crates/ritk-snap/src/render/gpu_volume/tests_gpu_volume.rs`; acceptance extension: the browser trace validator and GPU renderer stay below the 500-line source target, and browser attribute parsing has no type-suffixed function names. Behavior and the committed real-image evidence remain unchanged.
...
- Completed increment (2026-09-12): the explicit `orthogonal-with-mip` layout now has a complete visible Métis HWND capture of the same public 409-file CT. Two independent launches produced a byte-identical 1296 × 839 PNG showing axial, coronal, sagittal, and axial-MIP panels; [the image and provenance](docs/manual/dicom-workflow.md#capture-the-complete-mip-application-window) bind the source, executable, stack revisions, panel counts and orderly close. Private studies remain local; GPU, cross-engine browser input and installed package evidence remain open.
