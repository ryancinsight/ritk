...
- Action-adapter increment: `ViewerViewport` maps validated display placement into RITK image coordinates; the eframe shell now feeds its pointer response through the shared event dispatcher and applies actions to `SnapApp`. `ImagePoint` and `ViewportOffset` remove egui point carriers from interaction state. Corrections preserve multi-click measurements, accumulate repaint requests, cancel lost-pointer gestures, carry fractional display coordinates without rounding, and subtract large viewport origins before image-space narrowing. Verification: `ritk-snap` nextest 775/775, strict Clippy, formatting and package checks pass; DICOM loaders and clinical state remain RITK-owned.
