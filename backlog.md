...
- Status: in-progress; priority: P1; owner: RITK viewer + Métis framework; integrator: root; branch: `feat/ritk-native-viewer-session`; last-update: 2026-09-10; risk: lost viewer behavior; decision: [ADR 0026](docs/adr/0026-viewer-presentation-migration.md).
...
- Claim: root; scope: one interactive native Métis viewer session over the existing RITK DICOM loader, presentation frame and action adapter; branch: `feat/ritk-native-viewer-session`; DICOM remains RITK-owned and no parser or volume state crosses the host seam.
...
- Native session increment: the Windows `ritk-snap PATH --metis-native` path loads a real RITK study, renders the active slice through Métis's bounded native framebuffer, applies translated events, handles resize/minimize, DPI, focus cancellation and terminal lifecycle, and supports a finite hidden source-frame capture. Focused value tests pass on the session; the increment remains in progress until the full local gate, Windows visual capture and hosted merge checks complete.
