...
- Completed increment: explicit SeriesInstanceUID startup selection now verifies RITK discovery membership before decoding, and eframe capture waits for the deferred load instead of failing on its first empty frame; unknown or ambiguous inputs fail closed. Verification: 816/816 `ritk-snap` nextest, strict all-target Clippy, fmt, doctests and rustdoc; saved local DICOM eframe and Métis captures exit 0, while no-selection and unknown-selection probes exit 1. Private captures remain ignored.
