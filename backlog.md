...
- Cross-engine workflow increment 2026-09-14: `.github/workflows/metis-browser-dicom.yml` now builds the lock-pinned RITK WebAssembly consumer and Métis gallery, then runs the saved 94-file public MRI-DIR T2 study through the standard W3C chooser on Chromium, Firefox, and WebKit. The matrix asserts the existing RITK RGBA oracle, canvas dimensions and semantic `data-ritk-*` attributes, bounded rejection probes, and clean WebDriver teardown; hosted artifacts remain pending the first scheduled or manual run.
