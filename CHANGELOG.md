...
- [patch] `ritk_filter::mppca::MpPcaDenoiser` decomposes each window's Gram
  ...
  volumes denoises in 1.63 s instead of 58.7 s in parallel, and in 26.2 s
  instead of 1033 s on one core. f64 output agrees with the Jacobi path to
  3.5e-11 with identical signal ranks; at f32 the rank differs in 1.4% of
...
- Centralized the Reqwest transport policy in the workspace dependency table,
  upgraded it to 0.13.4, and disabled its native-TLS default. RITK network
  clients now use the maintained rustls stack without linking the system
  OpenSSL transport or retaining the vulnerable rustls-webpki 0.101 line.
...
- `ritk-interpolation` 0.4.0 now expresses native flat-buffer trilinear
  ...
  CI now checks out merged Eunomia commit `a2e4f390`, which owns the required
  generic float-to-index conversion.
...
- The RITK Apollo requirement and lockfile now consume the current 0.25.0
  provider release, and the CI checkout selects Apollo main at
  `c8742814be8c01f925aa8ead77c215ebbb9ff66f` to keep downstream path-provider
  resolution coherent. Remove unused Hephaestus workspace dependencies, patch
  entries, and `patch.unused` lock metadata that emitted Cargo warnings without
  serving a RITK consumer.
